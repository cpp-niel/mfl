...
namespace mfl
{
    namespace
    {
        ...
        vlist make_fraction_vlist(const dist_t d, box&& b)
        {
            return make_vlist(kern{.size = d}, std::move(b));
        }
        ...
        node_variant make_delimiter(const settings s, const code_point delimiter_char_code, const dist_t requested_height)
        {
            if (delimiter_char_code)
                                      ...
                                      const code_point right_delimiter_char_code, box&& numerator, box&& denominator)
        {
            ...
            auto middle = thickness ? make_fraction(s, thickness, width, std::move(num), std::move(den))
